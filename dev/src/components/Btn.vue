...
const modelValue = defineModel<boolean | null>({ local: true, default: null });
...
function setRippleOrigin(ev: PointerEvent) {
  if (ev.target) {
    ...
    requestAnimationFrame(() => {
      classList.remove('sv-ripple--enabled');
    });
  }
...
<template>
  <component :is="tagName" v-bind="bindProps">
    <div class="sv-btn__state" :class="{ 'sv-ripple': showRipple }">
...
.sv-btn
  @apply inline-flex transition-colors duration-300

  &__state
    @apply relative overflow-hidden inline-flex b-rd-inherit
    ...
    &.sv-ripple
      &:after
        @apply absolute w-100% h-100% opacity-0
        content: ""
  ...
  &__outline
    @apply absolute inset-0 b-rd-inherit
  ...
  &--icon
    @apply outline-none b-rd-50%

    > .sv-btn__state
      @apply pa-2 ma-1 select-none

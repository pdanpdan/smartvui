...
describe('should return theme attribute', () => {
  ...
  it('should set theme `auto` on null', () => {
    expect.soft(attrs.value).toEqual({ 'data-sv-theme': 'auto' });
  });

  it('should set theme `light` on false', async () => {
    isDark.value = false;
    await nextTick();
    expect.soft(attrs.value).toEqual({ 'data-sv-theme': 'light' });
  });

  it('should set theme `dark` on true', async () => {
    isDark.value = true;
    await nextTick();
    expect.soft(attrs.value).toEqual({ 'data-sv-theme': 'dark' });
  });
});
...
describe('should return theme CSS variables', () => {
  ...
  it('should return both themes on `auto`', () => {
    expect.soft(style.value).toHaveProperty('--md-sys-color-primary-light', '#7b580d');
    expect.soft(style.value).toHaveProperty('--md-sys-color-smartvui-light', '#006879');
    expect.soft(style.value).toHaveProperty('--md-sys-color-primary-dark', '#eebf6d');
    expect.soft(style.value).toHaveProperty('--md-sys-color-smartvui-dark', '#54d7f2');
  ...
  it('should return light theme on `light`', async () => {
    isDark.value = false;
    await nextTick();
    expect.soft(style.value).toHaveProperty('--md-sys-color-primary', '#7b580d');
    expect.soft(style.value).toHaveProperty('--md-sys-color-smartvui', '#006879');
    expect.soft(style.value).not.toHaveProperty('--md-sys-color-primary-light');
    expect.soft(style.value).not.toHaveProperty('--md-sys-color-primary-dark');
  ...
  it('should return dark theme on `dark`', async () => {
    isDark.value = true;
    await nextTick();
    expect.soft(style.value).toHaveProperty('--md-sys-color-primary', '#eebf6d');
    expect.soft(style.value).toHaveProperty('--md-sys-color-smartvui', '#54d7f2');
    expect.soft(style.value).not.toHaveProperty('--md-sys-color-primary-light');
    expect.soft(style.value).not.toHaveProperty('--md-sys-color-primary-dark');
    expect.soft(Object.entries(style.value)).toHaveLength(60);
  });
  ...
  it('should default to hex', async () => {
    ...
    expect.soft(styleHex.value).toHaveProperty('--md-sys-color-primary', '#eebf6d');
  });

  it('should generate palette tones', () => {
    expect.soft(style.value).toHaveProperty('--md-ref-palette-primary10', '#271900');
    expect.soft(style.value).toHaveProperty('--md-ref-palette-smartvui10', '#001f26');
  });

  it('should not generate palette tones by default', async () => {
    const stylePalette = useThemeStyle({ theme, isDark }).style;
    expect.soft(stylePalette.value).not.toHaveProperty('--md-ref-palette-primary10');
  ...
  it('should filter using strings or regex list', () => {
    ...
    expect.soft(styleFiltered.value).not.toHaveProperty('--md-sys-color-secondary');
  ...
  it('should filter using function', () => {
    ...
    expect.soft(styleFiltered.value).not.toHaveProperty('--md-sys-color-tertiary');
    expect.soft(styleFiltered.value).toHaveProperty('--md-sys-color-secondary');
  ...
  it('should change color', async () => {
    source.value = '#00f8b2';
    await nextTick();
    expect.soft(style.value).toHaveProperty('--md-sys-color-primary', '#8dd5b2');
  });

  it('should return RGB color when requested', async () => {
    format.value = 'rgb';
    await nextTick();
    expect.soft(style.value).toHaveProperty('--md-sys-color-primary', '141 213 178');
    expect.soft(style.value).toHaveProperty('--md-sys-color-smartvui', '84 215 242');
  });

  it('should change custom colors', async () => {
    customColors.value = [];
    await nextTick();
    expect.soft(style.value).not.toHaveProperty('--md-sys-color-smartvui');
  });
  ...
  it('should not generate custom colors when not needed', async () => {
    ...
    expect.soft(themeNoCustom.value.customColors).toEqual([]);
  });
});

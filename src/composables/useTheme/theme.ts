...
import type {
  CustomColorGroup as CustomColorGroupOrig,
  CustomColor as CustomColorOrig,
  TonalPalette,
} from '@material/material-color-utilities';
 ...
 * @public
 */
export type CustomColor = Omit<CustomColorOrig, 'value'> & {
 ...
 * @public
 */
export type CustomColorGroup = CustomColorGroupOrig & {
  /** Tonal palette. */
 ...
 * @see {@link https://github.com/material-foundation/material-color-utilities/blob/main/make_schemes.md#swift-1}
 */
export type ThemeSchemeVariants = keyof typeof schemes;

/**
 * Options for `themeFromSourceColor`.
 * @public
 */
...
/**
 * Generate a Material Design 3 `theme` from a `source` color.
 * @public
...
export function themeFromSourceColor(
  ...
  const light = new schemes[ options.variant ?? 'TonalSpot' ](Hct.fromInt(sourceNum), false, options.contrastLevel ?? 0);
  const dark = new schemes[ options.variant ?? 'TonalSpot' ](Hct.fromInt(sourceNum), true, options.contrastLevel ?? 0);
